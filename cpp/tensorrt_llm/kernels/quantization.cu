    ...
    bool useUE8M0, QuantizationSFLayout layout, int multiProcessorCount, cudaStream_t stream)
{
...
#endif
    {
        ...
        attrs[0].val.programmaticStreamSerializationAllowed = tensorrt_llm::common::getEnvEnablePDLForKernel("TRTLLM_DISABLE_PDL_QUANT_FP4");
        config.numAttrs = 1;
        config.attrs = attrs;
    ...
    QuantizationSFLayout layout, int multiProcessorCount, cudaStream_t stream)
{
    ...
    cudaLaunchAttribute attrs[1];
    attrs[0].id = cudaLaunchAttributeProgrammaticStreamSerialization;
    attrs[0].val.programmaticStreamSerializationAllowed = tensorrt_llm::common::getEnvEnablePDLForKernel("TRTLLM_DISABLE_PDL_QUANT_MXFP8");
    config.numAttrs = 1;
    config.attrs = attrs;
    ...
    float* globalScale, int multiProcessorCount, cudaStream_t stream)
{
    ...
    cudaLaunchAttribute attrs[1];
    attrs[0].id = cudaLaunchAttributeProgrammaticStreamSerialization;
    attrs[0].val.programmaticStreamSerializationAllowed = tensorrt_llm::common::getEnvEnablePDLForKernel("TRTLLM_DISABLE_PDL_QUANT_FP4_GLOBAL_SCALE");
    config.numAttrs = 1;
    config.attrs = attrs;
