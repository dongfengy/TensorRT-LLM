...
class TestPiecewiseCudaGraphCaptureDefaults:
    ...
    3. Candidates above the reachable ceiling are clamped down to the
       ceiling rather than dropped (a requested 128
       becomes 127), and no size beyond the user's list is ever invented
       (a force-appended far ceiling made runtime padding
       execute the full ceiling shape for every iteration in the gap).
    """
    ...
    def test_piecewise_filter_never_invents_far_ceiling(self):
        ...
        Force-appending the reachable ceiling (e.g. 65536 over a list
        topping out at 13914) makes runtime padding execute every
        iteration in the gap as the full ceiling shape. The filter must
        never invent sizes the user did not request.
        """
    ...
    def test_piecewise_filter_drops_entries_above_reachable_ceiling(self):
        ...
        graph. They are clamped down to the ceiling and surfaced in
    ...
    def test_piecewise_filter_keeps_small_candidates_unchanged(self):
        ...
        Previously the ceiling (1016 here) was force-appended, which made
        runtime padding round every 9..1016-token iteration up to the
        1016-token graph. Nothing is invented anymore;
        iterations above 8 tokens run eagerly at their true size.
        """
        from tensorrt_llm._torch.pyexecutor.model_engine import \
            _filter_piecewise_capture_num_tokens
